function [ newPopulation2 ] = Eg1_elitism(population , newPopulation, Er, chrom_range)
...
for k = Elite_no + 1 :  M
    if k <= round(M*(1-0.5*Er))
        ...
        [newPopulation2.Chromosomes(k).fitness, newPopulation2.Chromosomes(k).R] = Eg1_CalFitness(newPopulation2.Chromosomes(k).Gene(:), k);
    end
end